...
Note that the latter four classes are empty, by definition. In practical terms, this means this algorithm yields the same result as the merge algorithm. Detecting and omitting empty classes may be a useful future optimization.
...
1. Treeify. Apply either _merge_ or _bypass-reduce-isolate_ iteratively until the result is a tree.
1. Find Sibling Subclasses. Traverse the tree and, for each vertex, enumerate its sibling subclasses.
1. Assert Disjointness. For each set of sibling subclasses, asset a `DisjointClasses()` axiom. For the merge algorithm, the resulting disjointness axioms involve singleton classes and union expressions, which are created in OWL with `ObjectUnionOf()` expressions. For the bypass-reduce-isolate algorithm, the resulting expressions involve unions, intersections, and complements. The latter two are created in OWL with `ObjectIntersectionOf()` and `ObjectComplementOf()`.
